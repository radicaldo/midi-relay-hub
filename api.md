...
midi-relay has both a REST-based API as well as a socket.io API.

By default, both run on Port `8090` in this fork (configurable via the `apiPort` setting).
...
Returns a snapshot of currently registered surfaces (see **Surfaces (Socket.IO)** below).
...
### REST: `/integrations/screendeck`: GET

Returns the persisted ScreenDeck integration settings.
...
```javascript
{
  screenDeck: {
    ...
    companionPort: 16622,
    emulatorUrl: 'http://10.77.77.150:8000/emulator/',
    devices: []
  }
}
```

### REST: `/integrations/screendeck`: POST

Updates ScreenDeck integration settings. All fields are optional; values are sanitized.

```javascript
// request body
{
  companionHost: '10.77.77.150',
  companionPort: 16622,
  emulatorUrl: 'http://10.77.77.150:8000/emulator/',
  devices: [
    {
...
This fork adds a lightweight “surface registry” over Socket.IO. External clients can register a surface and (optionally) push draw/key state.

### socket.io: `surface_register` (client → server)

Registers/updates a surface.

```javascript
{
  surfaceId: 'ThinkPadDeck',
  name: 'ThinkPadDeck',
  cols: 8,
  rows: 4
}
...
### socket.io: `surface_draw` (client → server)

Pushes a surface “frame” / tile data payload.

```javascript
{
  surfaceId: 'ThinkPadDeck',
  frame: {
    // implementation-defined: key images/states
  }
}
```

### socket.io: `surfaces` (server → client)

Broadcasts a snapshot whenever the registry changes.
...
In this fork, the default UI URL is `http://127.0.0.1:8090/`.
