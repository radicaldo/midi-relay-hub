...
function profilesRenderSelect() {
	...
	availableProfiles.forEach((p) => {
		...
		opt.value = p.name
		opt.textContent = p.triggerCount != null ? `${p.name} (${p.triggerCount})` : p.name
		sel.appendChild(opt)
	})
}

function profilesFetchList() {
	return fetch('/profiles')
		.then((r) => r.json())
		.then((data) => {
			availableProfiles = (data && data.profiles) || []
			profilesRenderSelect()
			return availableProfiles
		})
}

function profilesSelectChanged() {
	const sel = document.getElementById('profileSelect')
	if (!sel || !sel.value) return
	setProfileStatus(`Selected: ${sel.value}`, false)
}

function profilesSaveCurrent() {
	const nameEl = document.getElementById('profileName')
	const name = nameEl ? nameEl.value.trim() : ''
	if (!name) {
		setProfileStatus('Enter a profile name first.', true)
		return
	}

	setProfileStatus(`Saving profile “${name}”…`, false)
	fetch('/profiles/save', {
		method: 'POST',
		...
		.then((data) => {
			...
			setProfileStatus(`Saved: ${name}`, false)
			return profilesFetchList()
		})
		.then(() => {
			const sel = document.getElementById('profileSelect')
		...
		.catch((err) => {
			setProfileStatus(err && err.message ? err.message : String(err), true)
		})
}

function profilesLoadSelected() {
	const sel = document.getElementById('profileSelect')
	const name = sel ? sel.value : ''
	if (!name) {
		setProfileStatus('Choose a profile to load.', true)
		return
	}

	if (!confirm(`Load profile “${name}”? This will replace your current triggers.`)) return

	setProfileStatus(`Loading profile “${name}”…`, false)
	fetch('/profiles/load', {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
		body: JSON.stringify({ name }),
		...
		.then((data) => {
			...
			setProfileStatus(`Loaded: ${name}`, false)
			// triggers refresh will come from server broadcast, but we also request them in case
...
function profilesDeleteSelected() {
	const sel = document.getElementById('profileSelect')
	const name = sel ? sel.value : ''
	if (!name) {
		setProfileStatus('Choose a profile to delete.', true)
		return
	}
	if (!confirm(`Delete profile “${name}”?`)) return

	setProfileStatus(`Deleting profile “${name}”…`, false)
...
const DASHBOARD_PANELS = ['panelTriggers', 'panelActivity', 'panelSurfaces', 'panelIntegrations', 'panelProfiles']
...
function surfacesSetStatus(msg, isError) {
	const el = document.getElementById('surfacesStatus')
	if (!el) return
	el.style.color = isError ? '#b91c1c' : '#374151'
	el.innerText = msg
}
...
function getSurfaceById(surfaceId) {
	return availableSurfaces.find((s) => s && s.surfaceId === surfaceId) || null
}

function surfacesSelect(surfaceId) {
...
function surfacesRenderList() {
	...
	if (!Array.isArray(availableSurfaces) || availableSurfaces.length === 0) {
		const empty = document.createElement('div')
		empty.className = 'surfaceEmpty'
		empty.textContent = 'No surfaces connected.'
	...
	availableSurfaces.forEach((s) => {
		...
		title.textContent = s.name || s.surfaceId
		btn.appendChild(title)

		const meta = document.createElement('div')
		meta.className = 'surfaceMeta'
		const dims = `${s.columns || '?'}×${s.rows || '?'}`
		const host = s.host ? ` • ${s.host}` : ''
		meta.textContent = `${dims}${host}`
...
function surfacesRenderPreview() {
	const preview = document.getElementById('surfacePreview')
	if (!preview) return

	preview.innerHTML = ''

	if (!selectedSurfaceId) {
		preview.textContent = 'Select a surface to preview.'
		preview.classList.add('surfacePreviewEmpty')
		return
	}

	const surface = getSurfaceById(selectedSurfaceId)
	if (!surface) {
		preview.textContent = 'Surface is no longer connected.'
		preview.classList.add('surfacePreviewEmpty')
		return
	}
	...
	for (let y = 0; y < rows; y++) {
		for (let x = 0; x < columns; x++) {
			...
			cell.className = 'surfaceKey'
			cell.dataset.x = String(x)
			cell.dataset.y = String(y)
			cell.title = `${surface.surfaceId} (${x},${y})`

			const state = keys[`${x},${y}`] || {}
			if (state && typeof state.bgColor === 'string' && state.bgColor.trim()) {
...
function surfacesApplyListUpdate(next) {
	availableSurfaces = Array.isArray(next) ? next : []

	if (selectedSurfaceId) {
		const stillThere = availableSurfaces.some((s) => s && s.surfaceId === selectedSurfaceId)
		if (!stillThere) selectedSurfaceId = null
	}

	if (!selectedSurfaceId && availableSurfaces.length > 0) {
		selectedSurfaceId = availableSurfaces[0].surfaceId
	}

	const count = availableSurfaces.length
	surfacesSetStatus(count === 1 ? '1 surface connected.' : `${count} surfaces connected.`, false)
	surfacesRenderList()
	surfacesRenderPreview()
}

function surfacesFetchList() {
	return fetch('/surfaces')
		.then((r) => r.json())
		.then((data) => {
			const list = (data && data.surfaces) || []
			surfacesApplyListUpdate(list)
			return list
		})
		.catch((err) => {
			surfacesSetStatus(err && err.message ? err.message : String(err), true)
		})
}
...
function showPanel(panelId, options = {}) {
	...
	for (const id of DASHBOARD_PANELS) {
		...
		const navId = 'nav' + id.substring('panel'.length)
		const navBtn = document.getElementById(navId)
		if (navBtn) {
			if (id === panelId) navBtn.classList.add('active')
	...
	if (persist) {
		try {
			localStorage.setItem('dashboardPanel', panelId)
		} catch (e) {
			// ignore
		}
	}
	...
	if (updateHash) {
	try {
			if (window.location.hash !== `#${panelId}`) window.location.hash = panelId
	} catch (e) {
		// ignore
	}
}
...
function initPanels() {
	let initial = 'panelTriggers'
	try {
		const hash = (window.location.hash || '').replace('#', '').trim()
		if (hash && DASHBOARD_PANELS.includes(hash)) initial = hash
		else {
			const stored = localStorage.getItem('dashboardPanel')
			if (stored && DASHBOARD_PANELS.includes(stored)) initial = stored
		}
	} catch (e) {
		// ignore
	}

	showPanel(initial, { updateHash: false, persist: false })
}
...
function onLoad() {
	...
	socket.on('connect', function () {
		document.getElementById('statusConnection').innerHTML = '<i class="fas fa-circle" style="color: #4CAF50;"></i> Connected'
		socket.emit('version') // Request version
		...
		fetch('/logs')
			.then((res) => res.json())
			.then((logs) => {
				let divActivityLog = document.getElementById('divActivityLog')
				let divMiniActivityLog = document.getElementById('divMiniActivityLog')

				if (divActivityLog) divActivityLog.innerHTML = ''
				if (divMiniActivityLog) divMiniActivityLog.innerHTML = ''

				// For mini log, only show last 50 entries
				const miniLogs = logs.slice(-50)

				logs.forEach((log) => {
					if (divActivityLog) {
						divActivityLog.appendChild(createLogEntry(log))
					}
				})

				miniLogs.forEach((log) => {
					if (divMiniActivityLog) {
						divMiniActivityLog.appendChild(createLogEntry(log))
					}
				})

				if (divActivityLog) divActivityLog.scrollTop = divActivityLog.scrollHeight
				if (divMiniActivityLog) divMiniActivityLog.scrollTop = divMiniActivityLog.scrollHeight
			})
	})

	socket.on('surfaces', function (list) {
		surfacesApplyListUpdate(list)
	})

	socket.on('surface_draw', function (evt) {
		if (!evt || !evt.surfaceId) return
		const surface = getSurfaceById(evt.surfaceId)
		if (!surface) return
		if (!surface.keys) surface.keys = {}
		surface.keys[`${evt.x},${evt.y}`] = evt.state || {}
		if (evt.surfaceId === selectedSurfaceId) {
			surfacesRenderPreview()
		}
	})
	...
	socket.on('triggers', function (triggers) {
		...
		tdHeaderName.innerHTML = '<b>Name</b>'
		...
		let tdHeaderTestButton = document.createElement('td')
		trHeader.appendChild(tdHeaderTestButton)
		...
		for (let i = 0; i < triggers.length; i++) {
			...
			// Name column
			let tdTriggerName = document.createElement('td')
			tdTriggerName.innerHTML = triggers[i].name || '<span style="color:#999">(unnamed)</span>'
			tdTriggerName.className = 'borderTop'
			tdTriggerName.style.fontWeight = triggers[i].name ? 'bold' : 'normal'
			trTrigger.appendChild(tdTriggerName)
			...
			let spanTriggerMIDIChannel,
				spanTriggerMIDINote,
				spanTriggerMIDIVelocity,
				spanTriggerMIDIController,
				spanTriggerMIDIValue,
				spanTriggerMIDISysExMessage

			switch (triggers[i].midicommand) {
				case 'noteon':
				case 'noteoff':
					spanTriggerMIDIChannel = document.createElement('span')
					spanTriggerMIDIChannel.innerHTML =
						'Channel: ' + (triggers[i].channel === '*' ? '*' : parseInt(triggers[i].channel) + 1)
					...
					spanTriggerMIDIVelocity.innerHTML =
						'Velocity: ' + (triggers[i].velocity === '*' ? '*' : parseInt(triggers[i].velocity))
				...
				case 'cc':
					...
					spanTriggerMIDIController.innerHTML =
						'Controller: ' + (triggers[i].controller === '*' ? '*' : parseInt(triggers[i].controller))
			...
			let tdBtnTestTrigger = document.createElement('td')
			tdBtnTestTrigger.className = 'borderTop'
			let btnTestTrigger = document.createElement('button')
			btnTestTrigger.innerHTML = 'Test'
			btnTestTrigger.setAttribute('onclick', "TestTrigger('" + triggers[i].id + "');")
			tdBtnTestTrigger.appendChild(btnTestTrigger)
			trTrigger.appendChild(tdBtnTestTrigger)
	...
	socket.on('disconnect', function () {
		...
		document.getElementById('statusConnection').innerHTML = '<i class="fas fa-circle" style="color: #d93025;"></i> Disconnected'
	})

	socket.on('version', function (ver) {
		document.getElementById('statusVersion').innerText = 'v' + ver
	})
}
...
function screenDeckEscapeHtml(s) {
	return String(s)
		.replaceAll('&', '&amp;')
		.replaceAll('<', '&lt;')
		.replaceAll('>', '&gt;')
...
function screenDeckDomIdSafe(id) {
	return String(id || '').replace(/[^a-zA-Z0-9_-]/g, '_')
}

function screenDeckSetStatus(msg, isError) {
	const el = document.getElementById('screenDeckEmulatorStatus')
	if (!el) return
	el.style.color = isError ? '#b91c1c' : '#374151'
	el.innerText = msg
}

function screenDeckSetSettingsStatus(msg, isError) {
	const el = document.getElementById('screenDeckSettingsStatus')
	if (!el) return
	el.style.color = isError ? '#b91c1c' : '#374151'
	el.innerText = msg
}

async function screenDeckFetchSettings() {
	const r = await fetch('/integrations/screendeck')
	if (!r.ok) throw new Error(`Failed to load ScreenDeck settings (${r.status})`)
	const data = await r.json()
	return (data && data.screenDeck) || {}
}

function screenDeckEnsureDeviceId(device) {
	if (device && typeof device.id === 'string' && device.id.trim()) return device.id.trim()
	try {
		return (crypto && crypto.randomUUID && crypto.randomUUID()) || `screendeck-${Date.now()}-${Math.floor(Math.random() * 1e6)}`
	} catch (_e) {
		return `screendeck-${Date.now()}-${Math.floor(Math.random() * 1e6)}`
	}
}

function screenDeckRenderDevices() {
	const root = document.getElementById('screenDeckDevices')
	if (!root) return
	const devices = Array.isArray(screenDeckState.devices) ? screenDeckState.devices : []
	if (devices.length === 0) {
		root.innerHTML = '<div style="font-size: 0.9em; color: #6b7280;">No ScreenDeck devices configured yet.</div>'
		return
	}

	root.innerHTML = devices
		.map((d) => {
			const id = screenDeckEnsureDeviceId(d)
			const domId = screenDeckDomIdSafe(id)
			const safeTitle = screenDeckEscapeHtml(id)
			const columns = Number.isFinite(Number(d.columns)) ? Number(d.columns) : 8
			const rows = Number.isFinite(Number(d.rows)) ? Number(d.rows) : 4
			const bitmap = Number.isFinite(Number(d.bitmap)) ? Number(d.bitmap) : 72
			const backgroundColor = typeof d.backgroundColor === 'string' ? d.backgroundColor : '#000000'
			const backgroundOpacity = Number.isFinite(Number(d.backgroundOpacity)) ? Number(d.backgroundOpacity) : 60
			const alwaysOnTop = !!d.alwaysOnTop
			const movable = !!d.movable
			const disableButtonPresses = !!d.disableButtonPresses

			return `
				<div class="screenDeckDeviceCard" id="sd_card_${screenDeckEscapeHtml(domId)}">
					<div class="screenDeckDeviceHeader">
						<div style="font-weight: 600;">${safeTitle}</div>
						<div style="display:flex; gap:8px; align-items:center;">
							<button onclick="screenDeckSaveDevice('${screenDeckEscapeHtml(id)}')">Save</button>
							<button onclick="screenDeckDeleteDevice('${screenDeckEscapeHtml(id)}')" style="background-color:#dc2626;">Delete</button>
						</div>
					</div>

					<div class="screenDeckDeviceGrid">
						<div>
							<label for="sd_${screenDeckEscapeHtml(domId)}_columns">Columns:</label>
							<input id="sd_${screenDeckEscapeHtml(domId)}_columns" type="number" min="1" max="32" value="${columns}" />
						</div>
						<div>
							<label for="sd_${screenDeckEscapeHtml(domId)}_rows">Rows:</label>
							<input id="sd_${screenDeckEscapeHtml(domId)}_rows" type="number" min="1" max="32" value="${rows}" />
						</div>
						<div>
							<label for="sd_${screenDeckEscapeHtml(domId)}_bitmap">Bitmap:</label>
							<input id="sd_${screenDeckEscapeHtml(domId)}_bitmap" type="number" min="1" max="512" value="${bitmap}" />
						</div>
						<div>
							<label for="sd_${screenDeckEscapeHtml(domId)}_bg">Background Color:</label>
							<input id="sd_${screenDeckEscapeHtml(domId)}_bg" type="text" value="${screenDeckEscapeHtml(backgroundColor)}" placeholder="#000000" />
						</div>
						<div>
							<label for="sd_${screenDeckEscapeHtml(domId)}_opacity">Background Opacity (0-100):</label>
							<input id="sd_${screenDeckEscapeHtml(domId)}_opacity" type="number" min="0" max="100" value="${backgroundOpacity}" />
						</div>
					</div>

					<div style="display:flex; gap:14px; flex-wrap:wrap; margin-top: 8px;">
						<label style="padding: 0;"><input id="sd_${screenDeckEscapeHtml(domId)}_alwaysOnTop" type="checkbox" ${alwaysOnTop ? 'checked' : ''} /> Always On Top</label>
						<label style="padding: 0;"><input id="sd_${screenDeckEscapeHtml(domId)}_movable" type="checkbox" ${movable ? 'checked' : ''} /> Movable</label>
						<label style="padding: 0;"><input id="sd_${screenDeckEscapeHtml(domId)}_disablePress" type="checkbox" ${disableButtonPresses ? 'checked' : ''} /> Disable Button Presses</label>
					</div>
				</div>
			`
		})
	...
	// If the settings accordion is currently open, resize it to fit the new content.
	const btn = document.getElementById('screenDeckSettingsAccordion')
...
function screenDeckRenderSettings(sd) {
	const hostEl = document.getElementById('screenDeckCompanionHost')
	const portEl = document.getElementById('screenDeckCompanionPort')
	if (hostEl) hostEl.value = (sd && typeof sd.companionHost === 'string' ? sd.companionHost : '127.0.0.1').trim()
	if (portEl) portEl.value = String(Number(sd.companionPort) || 16622)

	const devices = Array.isArray(sd && sd.devices) ? sd.devices : []
	screenDeckState.devices = devices.map((d) => ({ ...d, id: screenDeckEnsureDeviceId(d) }))
	screenDeckRenderDevices()
	screenDeckSetSettingsStatus('Loaded ScreenDeck settings.', false)
}

async function screenDeckLoadSettings() {
	try {
		const sd = await screenDeckFetchSettings()
		screenDeckRenderSettings(sd)
		const urlEl = document.getElementById('screenDeckEmulatorUrl')
		const url = (sd && typeof sd.emulatorUrl === 'string' ? sd.emulatorUrl : '').trim()
		if (urlEl) urlEl.value = url
		if (url) {
			screenDeckSetStatus('Loaded emulator URL from settings.', false)
			// Auto-load so the Surfaces tab shows the emulator immediately
			screenDeckLoadEmulator()
		} else {
			screenDeckSetStatus('(not configured)', false)
			const frame = document.getElementById('screenDeckEmulatorFrame')
			if (frame) frame.removeAttribute('src')
		}
	} catch (err) {
		screenDeckSetStatus(err && err.message ? err.message : String(err), true)
		screenDeckSetSettingsStatus(err && err.message ? err.message : String(err), true)
	}
}

async function screenDeckSaveConnection() {
	const hostEl = document.getElementById('screenDeckCompanionHost')
	const portEl = document.getElementById('screenDeckCompanionPort')
	const host = hostEl ? hostEl.value.trim() : ''
	const port = portEl ? portEl.value : ''

	screenDeckSetSettingsStatus('Saving…', false)
	try {
		const r = await fetch('/integrations/screendeck', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ companionHost: host, companionPort: port, devices: screenDeckState.devices }),
		})
...
function screenDeckAddDevice() {
	const id = screenDeckEnsureDeviceId({})
	screenDeckState.devices = [
		...((Array.isArray(screenDeckState.devices) ? screenDeckState.devices : []).filter((d) => d && d.id !== id)),
		{
			id,
			columns: 8,
			rows: 4,
	...
	screenDeckRenderDevices()
	screenDeckSetSettingsStatus('Added new device (not saved yet).', false)
}

function screenDeckReadDeviceFromDom(deviceId) {
	const id = String(deviceId || '').trim()
	if (!id) return null
	const domId = screenDeckDomIdSafe(id)
	const get = (suffix) => document.getElementById(`sd_${domId}_${suffix}`)
	const columns = Number.parseInt(String(get('columns')?.value ?? '8'), 10)
	const rows = Number.parseInt(String(get('rows')?.value ?? '4'), 10)
	const bitmap = Number.parseInt(String(get('bitmap')?.value ?? '72'), 10)
	const backgroundColor = String(get('bg')?.value ?? '#000000').trim()
	const backgroundOpacity = Number.parseInt(String(get('opacity')?.value ?? '60'), 10)
	const alwaysOnTop = !!get('alwaysOnTop')?.checked
	const movable = !!get('movable')?.checked
	const disableButtonPresses = !!get('disablePress')?.checked
...
async function screenDeckSaveDevice(deviceId) {
	const nextDevice = screenDeckReadDeviceFromDom(deviceId)
	if (!nextDevice) return
	const devices = Array.isArray(screenDeckState.devices) ? screenDeckState.devices : []
	screenDeckState.devices = [...devices.filter((d) => d && d.id !== nextDevice.id), nextDevice]
	...
	try {
		...
		const r = await fetch('/integrations/screendeck', {
			...
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({
				companionHost: hostEl ? hostEl.value.trim() : undefined,
				companionPort: portEl ? portEl.value : undefined,
				devices: screenDeckState.devices,
			}),
		})
		const data = await r.json().catch(() => ({}))
		if (!r.ok || !data || data.success === false) throw new Error((data && data.error) || 'Save failed')
...
async function screenDeckDeleteDevice(deviceId) {
	const id = String(deviceId || '').trim()
	if (!id) return
	const devices = Array.isArray(screenDeckState.devices) ? screenDeckState.devices : []
	screenDeckState.devices = devices.filter((d) => d && d.id !== id)
	screenDeckRenderDevices()
...
async function screenDeckSaveEmulatorUrl() {
	const urlEl = document.getElementById('screenDeckEmulatorUrl')
	const url = urlEl ? urlEl.value.trim() : ''

	screenDeckSetStatus('Saving…', false)
	try {
		const r = await fetch('/integrations/screendeck', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ emulatorUrl: url }),
		})
		const data = await r.json().catch(() => ({}))
		if (!r.ok || !data || data.success === false) throw new Error((data && data.error) || 'Save failed')
		screenDeckSetStatus('Saved.', false)
		// Load (sanitized) value back from server response
		const nextUrl = (data && data.screenDeck && data.screenDeck.emulatorUrl) || ''
		if (urlEl) urlEl.value = nextUrl
		if (nextUrl) screenDeckLoadEmulator()
		else {
			const frame = document.getElementById('screenDeckEmulatorFrame')
			if (frame) frame.removeAttribute('src')
		}
...
function screenDeckLoadEmulator() {
	const urlEl = document.getElementById('screenDeckEmulatorUrl')
	const url = urlEl ? urlEl.value.trim() : ''
	const frame = document.getElementById('screenDeckEmulatorFrame')
	if (!frame) return
	...
	if (!url) {
		screenDeckSetStatus('Enter an emulator URL first.', true)
		frame.removeAttribute('src')
		return
	}

	frame.src = url
	screenDeckSetStatus(`Loaded: ${url}`, false)
}

function screenDeckOpenEmulator() {
	const urlEl = document.getElementById('screenDeckEmulatorUrl')
	const url = urlEl ? urlEl.value.trim() : ''
	if (!url) {
		screenDeckSetStatus('Enter an emulator URL first.', true)
...
function TestTrigger(triggerID) {
	fetch('/trigger/test', {
		...
		.then((result) => {
			if (result.success) {
				alert('Trigger test initiated! Check the activity log or destination for results.')
			} else {
				alert('Error testing trigger: ' + result.error)
			}
		})
}
...
function applyTemplate() {
	...
	let txtJSON = document.getElementById('txtJSONData')
	let selectHTTPMethod = document.getElementById('selectHTTPMethod')

	switch (val) {
		case 'companion':
			txtURL.value = 'http://127.0.0.1:8888/api/location/1/1/1/press'
			if (selectHTTPMethod) selectHTTPMethod.value = 'POST'
			if (typeof companionBuilderUpdate === 'function') {
				try {
					document.getElementById('companionProtocol').value = 'http'
					document.getElementById('companionHost').value = '127.0.0.1'
					document.getElementById('companionPort').value = 8888
					document.getElementById('companionEndpointType').value = 'location-action'
					document.getElementById('companionPage').value = 1
					document.getElementById('companionRow').value = 0
					document.getElementById('companionColumn').value = 0
					document.getElementById('companionAction').value = 'press'
					companionBuilderUpdate()
				} catch (e) {
					// Ignore if builder is not present
				}
			}
			break
		case 'n8n':
			txtURL.value = 'http://localhost:5678/webhook/path-here'
			txtJSON.value = JSON.stringify({ note: 'C4', velocity: 100 }, null, 2)
			if (selectHTTPMethod) selectHTTPMethod.value = 'POST'
			break
		case 'generic':
			txtURL.value = 'http://localhost:3000/api/trigger'
			if (selectHTTPMethod) selectHTTPMethod.value = 'POST'
			break
	}
...
function ShowEditTrigger(id) {
	...
	for (let i = 0; i < triggers.length; i++) {
		if (triggers[i].id === id) {
			// Load trigger name
			document.getElementById('txtTriggerName').value = triggers[i].name || ''
			...
			switch (triggers[i].actiontype) {
				case 'http':
					...
					document.getElementById('selectHTTPMethod').value = triggers[i].method || ''
...
function AddTrigger() {
	...
	// Add trigger name
	triggerObj.name = document.getElementById('txtTriggerName').value.trim()
	...
	switch (triggerObj.actiontype) {
		case 'http':
			...
			triggerObj.method = document.getElementById('selectHTTPMethod').value
...
function EditTrigger() {
	...
	// Reset button text after saving
	let btnEditTrigger = document.getElementById('btnEditTrigger')
	btnEditTrigger.innerHTML = 'Edit Trigger'
...
function formReset() {
	// Clear trigger name
	document.getElementById('txtTriggerName').value = ''
...
function showHideAccordion(elOrId) {
	const el =
		(typeof elOrId === 'string' && document.getElementById(elOrId)) ||
		(elOrId && elOrId.classList ? elOrId : document.getElementById('bitfocusCompanionHelpers'))
	if (!el) return
...
function companionBuilderGetValue(id, fallback) {
	const el = document.getElementById(id)
...
function companionBuilderUpdate() {
	const protocol = companionBuilderGetValue('companionProtocol', 'http')
	const host = companionBuilderGetValue('companionHost', '127.0.0.1')
	const port = companionBuilderGetValue('companionPort', '8888')
	const endpointType = companionBuilderGetValue('companionEndpointType', 'location-action')

	const locationFields = document.getElementById('companionLocationFields')
	const locationActionFields = document.getElementById('companionLocationActionFields')
	const styleQueryFields = document.getElementById('companionStyleQueryFields')
	const styleBodyFields = document.getElementById('companionStyleBodyFields')
	const customVarFields = document.getElementById('companionCustomVarFields')
	const moduleVarFields = document.getElementById('companionModuleVarFields')

	if (locationFields) locationFields.style.display = endpointType.startsWith('location-') ? 'block' : 'none'
	if (locationActionFields) locationActionFields.style.display = endpointType === 'location-action' ? 'block' : 'none'
	if (styleQueryFields) styleQueryFields.style.display = endpointType === 'location-style-query' ? 'block' : 'none'
	if (styleBodyFields) styleBodyFields.style.display = endpointType === 'location-style-body' ? 'block' : 'none'
	if (customVarFields) customVarFields.style.display = endpointType.startsWith('custom-variable-') ? 'block' : 'none'
	if (moduleVarFields) moduleVarFields.style.display = endpointType === 'module-variable-get' ? 'block' : 'none'

	let method = 'POST'
	let url = `${protocol}://${host}:${port}`
	let jsonBody = ''

	if (endpointType.startsWith('location-')) {
		const page = companionBuilderGetValue('companionPage', '1')
		const row = companionBuilderGetValue('companionRow', '0')
		const column = companionBuilderGetValue('companionColumn', '0')
		url += `/api/location/${page}/${row}/${column}`

		if (endpointType === 'location-action') {
			const action = companionBuilderGetValue('companionAction', 'press')
			url += `/${encodeURIComponent(action)}`
			method = 'POST'
		} else if (endpointType === 'location-style-query') {
			const param = companionBuilderGetValue('companionStyleParam', 'text')
			const value = companionBuilderGetValue('companionStyleValue', 'TEST')
			url += `/style?${encodeURIComponent(param)}=${encodeURIComponent(value)}`
			method = 'POST'
		} else if (endpointType === 'location-style-body') {
			url += '/style'
			method = 'POST'
			const body = {}
			const text = companionBuilderGetValue('companionStyleText', '')
			const bgcolor = companionBuilderGetValue('companionStyleBg', '')
			const color = companionBuilderGetValue('companionStyleColor', '')
			const sizeRaw = companionBuilderGetValue('companionStyleSize', '')
			if (text !== '') body.text = text
			if (bgcolor !== '') body.bgcolor = bgcolor
			if (color !== '') body.color = color
			if (sizeRaw !== '' && !isNaN(parseInt(sizeRaw))) body.size = parseInt(sizeRaw)
			jsonBody = JSON.stringify(body, null, 2)
		}
	} else if (endpointType.startsWith('custom-variable-')) {
		const name = companionBuilderGetValue('companionCustomVarName', 'cue')
		url += `/api/custom-variable/${encodeURIComponent(name)}/value`

		if (endpointType === 'custom-variable-get') {
			method = 'GET'
		} else {
			const value = companionBuilderGetValue('companionCustomVarValue', 'intro')
			const mode = companionBuilderGetValue('companionCustomVarValueMode', 'text')
			method = 'POST'
			if (endpointType === 'custom-variable-set-query') {
				url += `?value=${encodeURIComponent(value)}`
			} else if (endpointType === 'custom-variable-set-body') {
				if (mode === 'json') {
					try {
						jsonBody = JSON.stringify(JSON.parse(value), null, 2)
					} catch (e) {
						jsonBody = JSON.stringify(value)
					}
				} else {
					jsonBody = JSON.stringify(value)
				}
	...
	} else if (endpointType === 'module-variable-get') {
		const label = companionBuilderGetValue('companionModuleLabel', 'My Connection')
		const name = companionBuilderGetValue('companionModuleVar', 'name')
		url += `/api/variable/${encodeURIComponent(label)}/${encodeURIComponent(name)}/value`
		method = 'GET'
	} else if (endpointType === 'surfaces-rescan') {
		url += '/api/surfaces/rescan'
		method = 'POST'
	}

	const preview = document.getElementById('companionBuiltUrl')
	if (preview) preview.innerHTML = url

	const methodEl = document.getElementById('companionBuiltMethod')
	if (methodEl) methodEl.innerText = method

	const jsonRow = document.getElementById('companionBuiltJsonRow')
	const jsonEl = document.getElementById('companionBuiltJson')
	if (jsonRow && jsonEl) {
		if (jsonBody && jsonBody !== '{}' && jsonBody !== '""') {
			jsonEl.innerHTML = jsonBody
			jsonRow.style.display = 'flex'
		} else if (jsonBody && jsonBody.trim() !== '') {
			jsonEl.innerHTML = jsonBody
			jsonRow.style.display = 'flex'
		} else {
			jsonEl.innerHTML = '{}'
...
function companionBuilderApply() {
	companionBuilderUpdate()
	const preview = document.getElementById('companionBuiltUrl')
	if (!preview) return

	document.getElementById('txtURL').value = preview.innerHTML
	const selectHTTPMethod = document.getElementById('selectHTTPMethod')
	const methodEl = document.getElementById('companionBuiltMethod')
	if (selectHTTPMethod && methodEl) selectHTTPMethod.value = methodEl.innerText

	const jsonRow = document.getElementById('companionBuiltJsonRow')
	const jsonEl = document.getElementById('companionBuiltJson')
	const txtJSON = document.getElementById('txtJSONData')
	if (txtJSON && jsonRow && jsonEl && jsonRow.style.display !== 'none') {
		txtJSON.value = jsonEl.innerHTML
	} else if (txtJSON) {
		txtJSON.value = ''
	}
}
...
function createLogEntry(log) {
	...
	let color = '#333'
	if (log.direction === 'RX') color = '#1a73e8' // Blue
	else if (log.direction === 'TX') color = '#d93025' // Red
	else if (log.direction === 'TRIGGER') color = '#9334e6' // Purple
	else if (log.direction === 'APP-ERR') color = '#e37400' // Orange

	let time = new Date(log.timestamp).toLocaleTimeString()
	p.innerHTML = `<span style="color: #666">[${time}]</span> <strong style="color: ${color}">${log.direction}</strong> [${log.port}] ${log.command}: ${JSON.stringify(log.data)}`
	return p
}

function appendToActivityLogs(log) {
	// Add to main activity log
	let divActivityLog = document.getElementById('divActivityLog')
	if (divActivityLog) {
		divActivityLog.appendChild(createLogEntry(log))
		divActivityLog.scrollTop = divActivityLog.scrollHeight
	}

	// Add to mini activity log on Triggers page
	let divMiniActivityLog = document.getElementById('divMiniActivityLog')
	if (divMiniActivityLog) {
		divMiniActivityLog.appendChild(createLogEntry(log))
		divMiniActivityLog.scrollTop = divMiniActivityLog.scrollHeight
		// Keep mini log limited to last 50 entries
		while (divMiniActivityLog.children.length > 50) {
			divMiniActivityLog.removeChild(divMiniActivityLog.firstChild)
		}
	}
}

socket.on('midi_log', function (log) {
	appendToActivityLogs(log)
})

function clearLog() {
	let divActivityLog = document.getElementById('divActivityLog')
	if (divActivityLog) {
		divActivityLog.innerHTML = ''
	}
	let divMiniActivityLog = document.getElementById('divMiniActivityLog')
	if (divMiniActivityLog) {
		divMiniActivityLog.innerHTML = ''
	}
}
